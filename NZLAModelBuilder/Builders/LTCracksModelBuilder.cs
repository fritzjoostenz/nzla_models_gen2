...
{
    internal class LTCracksModelBuilder : ClassificationModelBuilderBase
    {

        public LTCracksModelBuilder(string workFolder, List<RoadSegmentBase> segments, Double testDataFraction = 0.25)
        {            
            this.WorkFolder = workFolder;

            this.SetupFilePaths("lt_cracks");
                        
            this.Segments = segments;
            foreach (var segment in segments)
            {
                segment.Target_Classification = false;
                if (segment.PctLTCracks > 0) { segment.Target_Classification = true; }
            }
