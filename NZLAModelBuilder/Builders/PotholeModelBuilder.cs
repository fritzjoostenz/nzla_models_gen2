...
{
    internal class PotholeModelBuilder : ClassificationModelBuilderBase
    {

        public PotholeModelBuilder(string workFolder, List<RoadSegmentBase> segments, Double testDataFraction = 0.25)
        {            
            this.WorkFolder = workFolder;

            this.SetupFilePaths("potholes");            
            ...
            foreach (var segment in segments)
            {
                ...
                if (segment.PctPotholes > 0) { segment.Target_Classification = true; }
            }
        ...
        protected override IEstimator<ITransformer> GetModelSetupPipeLine()
        {
            var pipeline = mlContext.Transforms.CopyColumns(outputColumnName: "Label", inputColumnName: nameof(RoadSegmentBase.Target_Classification))
                                     ...
                                     nameof(RoadSegmentBase.PctScabbing), nameof(RoadSegmentBase.PctLTCracks), nameof(RoadSegmentBase.PctAlligatorCracks)));
