...
{
    internal class MeshCracksModelBuilder : ClassificationModelBuilderBase
    {

        public MeshCracksModelBuilder(string workFolder, List<RoadSegmentBase> segments, Double testDataFraction = 0.25)
        {            
            this.WorkFolder = workFolder;
            
            this.SetupFilePaths("mesh_cracks");
            
            this.Segments = segments;
            foreach (var segment in segments)
            {
                segment.Target_Classification = false;
                if (segment.PctAlligatorCracks > 0) { segment.Target_Classification = true; }
            }
        ...
        protected override IEstimator<ITransformer> GetModelSetupPipeLine()
        {
            var pipeline = mlContext.Transforms.CopyColumns(outputColumnName: "Label", inputColumnName: nameof(RoadSegmentBase.Target_Classification))
                                     ...
                                     nameof(RoadSegmentBase.PctScabbing), nameof(RoadSegmentBase.PctLTCracks)));
            return pipeline;
        }
