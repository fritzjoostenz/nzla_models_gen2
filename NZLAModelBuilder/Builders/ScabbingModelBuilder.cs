...
{
    internal class ScabbingModelBuilder : ClassificationModelBuilderBase
    {

        public ScabbingModelBuilder(string workFolder, List<RoadSegmentBase> segments, Double testDataFraction = 0.25)
        {            
            this.WorkFolder = workFolder;

            this.SetupFilePaths("scabbing");
                        
            this.Segments = segments;
            foreach (var segment in segments)
            {
                segment.Target_Classification = false;
                if (segment.PctScabbing > 0) { segment.Target_Classification = true; }
            }
