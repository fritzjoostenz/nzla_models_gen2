...
{
    internal class RuttingModelBuilder : ClassificationModelBuilderBase
    {

        public RuttingModelBuilder(string workFolder, List<RoadSegmentBase> segments, Double testDataFraction = 0.25)
        {            
            this.WorkFolder = workFolder;
            
            this.SetupFilePaths("rutting");
            ...
            foreach (var segment in segments)
            {
                ...
                if (segment.Rut85th > 10) { segment.Target_Classification = true; }
            }
        ...
        protected override IEstimator<ITransformer> GetModelSetupPipeLine()
        {
            var pipeline = mlContext.Transforms.CopyColumns(outputColumnName: "Label", inputColumnName: nameof(RoadSegmentBase.Target_Classification))
                                     ...
                                     nameof(RoadSegmentBase.HeavyPercent), 
                                     nameof(RoadSegmentBase.SurfThickness), 
                                     nameof(RoadSegmentBase.SurfLayerCount), 
                                     nameof(RoadSegmentBase.PavementAge),
                                     nameof(RoadSegmentBase.SurfAge), 
                                     nameof(RoadSegmentBase.Length),
                                     nameof(RoadSegmentBase.PctAlligatorCracks), 
                                     nameof(RoadSegmentBase.PctShoving)));
            return pipeline;
        }
